...
{
    [RoutePrefix("DEL_PASS")]
    public class DelPassController : ApiController
    {
        [Route("GET_DS_DEL_PASS")]
        [HttpGet]
        public async Task<IHttpActionResult> GetDSDelPass(string Mobile, string Password,int P_Count )
        {
        ...
        [HttpGet]
        public async Task<IHttpActionResult> UpdateDelPass(string Mobile,string Password,int P_ID,string P_Account,int P_Status,string P_Descript)
        {
            if (!Datacs.IsValidMobile(Mobile, Password)) return Unauthorized();
            List<OracleParameter> param = new List<OracleParameter>();
